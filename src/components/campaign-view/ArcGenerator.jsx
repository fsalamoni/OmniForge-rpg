...
export default function ArcGenerator({ campaignId, description, answers5W2H, hooks = [], systemRpg, setting, onArcGenerated }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const hooksContext = hooks && hooks.length > 0
        ? `\nGANCHOS DA CAMPANHA (use-os como base para os conflitos e motivações do arco):\n${hooks.slice(0, 5).map((h, i) => `${i + 1}. ${typeof h === 'string' ? h : JSON.stringify(h)}`).join('\n')}`
        : '';
