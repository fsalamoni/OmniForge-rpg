...
import { Campaign, NpcCreature, CampaignLike } from '@/firebase/db';
...
export default function Library() {
  ...
  const { data: allCampaigns = [], isLoading } = useQuery({
    ...
    queryFn: () => Campaign.listPublic()
  });

  const { data: likes = [] } = useQuery({
    queryKey: ['my-likes', user?.uid],
    queryFn: () => CampaignLike.listByUser(user.uid),
    enabled: !!user
  });
  ...
  const cloneMutation = useMutation({
    mutationFn: async (originalCampaign) => {
      ...
      const originalNpcs = await NpcCreature.listByCampaign(originalCampaign.id);
      await Promise.all(originalNpcs.map(npc =>
        NpcCreature.create({
      ...
      await Campaign.incrementCloneCount(originalCampaign.id);
      return newCampaign;
    },
  ...
  const toggleLikeMutation = useMutation({
    mutationFn: (campaignId) => CampaignLike.toggle(user.uid, campaignId),
    onSuccess: () => {
      queryClient.invalidateQueries(['my-likes', user?.uid]);
    }
  });
  ...
  if (sortBy === 'popular') {
    filteredCampaigns = [...filteredCampaigns].sort((a, b) => (b.clone_count || 0) - (a.clone_count || 0));
  }
  ...
  const isLiked = (campaignId) => likes.some(l => l.campaignId === campaignId);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-4xl font-bold text-white">Biblioteca Pública</h1>
          </div>
          <p className="text-slate-400 text-lg">Explore e clone campanhas criadas pela comunidade</p>
        </div>
      </div>
      ...
      <div className="p-6 bg-slate-900/50 backdrop-blur-xl border border-purple-900/20 rounded-2xl space-y-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button
            ...
            size="sm"
            className={filterDuration === 'all' ? 'bg-purple-600 hover:bg-purple-700' : 'border-slate-700 text-slate-300 hover:bg-slate-800'}
          >
            Todas
          </Button>
          {uniqueDurations.map(duration => (
            <Button
              ...
              size="sm"
              className={filterDuration === duration ? 'bg-purple-600 hover:bg-purple-700' : 'border-slate-700 text-slate-300 hover:bg-slate-800'}
            >
              {duration}
            </Button>
          ))}
                ...
                <div className="flex gap-2">
                  ...
                  {user && (
                    <Button
                      ...
                      variant="outline"
                      className={`border-slate-700 ${isLiked(campaign.id) ? 'text-red-400 hover:text-red-300' : 'text-slate-400 hover:text-white'}`}
                    >
                      <Heart className={`w-4 h-4 ${isLiked(campaign.id) ? 'fill-current' : ''}`} />
                    </Button>
                  )}
                </div>
              </div>
